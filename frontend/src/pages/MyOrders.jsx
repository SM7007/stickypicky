  ...
  return (
    <MainLayout>
      <div className="max-w-4xl mx-auto px-4 py-10">
        <h1 className="text-3xl font-bold font-display text-white mb-10 uppercase tracking-wider">My Orders</h1>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 border border-dashed border-border rounded-lg bg-surface">
            ...
            <a
              href="/shop"
              className="bg-white text-black font-bold uppercase tracking-wider text-[10px] px-6 py-3 rounded hover:bg-zinc-200 transition-colors inline-block"
            >
              Shop Posters
            </a>
          </div>
          ...
          <div className="space-y-6">
            {orders.map((order) => (
              <div key={order.id} className="bg-surface border border-border rounded-lg overflow-hidden">
                {/* Header Summary */}
                <div className="p-4 sm:p-6 bg-[#161616]/40 border-b border-border flex flex-wrap justify-between items-center gap-4 text-xs text-secondary">
                  <div className="flex gap-6">
                    <div>
                      <span className="font-bold text-white uppercase block mb-1">Order Placed</span>
                    ...
                    <div>
                      <span className="font-bold text-white uppercase block mb-1">Total Paid</span>
                      <span className="text-white font-semibold">{formatPrice(order.totalAmount)}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div>
                      <span className="font-bold text-white uppercase block mb-1 text-right">Order ID</span>
                      <span className="font-mono text-white/70 block">{order.id}</span>
                    </div>
                ...
                <div className="px-4 py-3 bg-[#111111]/20 border-t border-border/30 text-[10px] text-secondary flex justify-between">
                  <span>Shipping Address: {order.customerName}, {order.city} ({order.pincode})</span>
                  <span>Payment status: <span className="font-bold text-white">{order.paymentStatus}</span></span>
                </div>
              </div>
            ))}
          </div>
